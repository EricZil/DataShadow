...
export default async function BreachesPage() {
  ...
  // Fetch pwned-data stats from the new API
  let pwnedDataCount: number | null = null;
  try {
    const res = await fetch('/api/stats/pwned-data', { cache: 'no-store' });
    if (res.ok) {
      const data = await res.json();
      pwnedDataCount = data.total;
    }
  } catch {
    pwnedDataCount = null;
  }
      ...
      <main className="flex-1 py-16 px-4 sm:px-6 lg:px-8 relative">
        ...
        <div className="container mx-auto max-w-7xl relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-16">
            ...
            <div className="group bg-gradient-to-br from-[#1A1F2E] to-[#1F2433] rounded-2xl p-8 border border-gray-800/50 hover:border-green-500/30 transition-all duration-300 hover:shadow-2xl hover:shadow-green-500/10">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="text-3xl font-bold text-green-400 group-hover:text-green-300 transition-colors">
                  {pwnedDataCount !== null ? pwnedDataCount.toLocaleString() : 'N/A'}
                </div>
              </div>
              <div className="text-gray-400 font-medium">Total Pwned Accounts</div>
            </div>
                  ...
                  <article key={post.id} className="group bg-gradient-to-br from-[#1A1F2E] to-[#1F2433] rounded-2xl p-8 border border-gray-800/50 hover:border-blue-500/30 transition-all duration-500 hover:shadow-2xl hover:shadow-blue-500/10 transform hover:-translate-y-1">
                    ...
                    <div className="pt-6 border-t border-gray-800/50">
                      <div className="flex items-center justify-between">
                        ...
                        <Link 
                          href={`/breaches/${post.id}`}
                          className="flex items-center justify-center px-6 py-3 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white rounded-xl font-semibold transition-all duration-300 transform hover:scale-105"
                        >
